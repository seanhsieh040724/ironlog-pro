...
export const MuscleHeatmap: React.FC<MuscleHeatmapProps> = ({ scores }) => {
  ...
  // Fix: Use 'as const' to ensure 'ease' is treated as a specific literal type for framer-motion, 
  // preventing it from being inferred as a generic 'string' which causes type errors.
  const transition = { duration: 0.8, ease: "circOut" as const };

  return (
    <div className="w-full flex flex-col items-center py-4 select-none">
      <div className="relative w-full max-w-[580px] aspect-[1.4] rounded-[32px] p-6 bg-[#f1f5f9] border border-slate-200 shadow-inner flex items-center justify-center overflow-hidden">
        
        {/* 視圖文字標籤 */}
        <div className="absolute top-6 left-0 right-0 flex justify-around pointer-events-none px-12">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] italic">正面</span>
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] italic">背面</span>
        </div>

        <svg viewBox="0 0 800 500" className="w-full h-full drop-shadow-sm">
          {/* 正面視圖 (Anterior) */}
          <g transform="translate(100, 50) scale(1.08)">
            {/* 頭部與頸部底色 */}
            ...
            <motion.path 
              ...
              strokeWidth="1.2" 
            />
            <line x1="100" y1="85" x2="100" y2="135" stroke="white" strokeWidth="0.8" />
            ...
            <motion.path animate={{ fill: getHeatColor(scores.shoulders) }} transition={transition}
              d="M140 82 C150 90 152 122 138 135 C132 120 132 98 140 82 Z" stroke="white" strokeWidth="1" />
          ...
          {/* 背面視圖 (Posterior) */}
          <g transform="translate(450, 50) scale(1.08)">
            {/* 頸部 */}
            <path d="M100 15 C95 15 92 18 92 25 V45 L100 55 L108 45 V25 C108 18 105 15 100 15" fill="#94a3b8" />
            <circle cx="100" cy="25" r="9" fill="#94a3b8" />
      ...
      <div className="mt-8 flex gap-12">
        {[0, 35, 75, 100].map((s, i) => (
          <div key={i} className="flex flex-col items-center gap-2">
            <div className="w-3 h-3 rounded-sm shadow-sm" style={{ backgroundColor: getHeatColor(s) }} />

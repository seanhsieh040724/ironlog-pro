        ...
        {!activeExerciseId ? (
          <motion.div key="overview" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6 pb-40">
            <div className="space-y-5 pt-2">
                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                  {Object.keys(ORGANIZED_EXERCISES).map(cat => (
                    <button 
                      ...
                      className={`shrink-0 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${activeCategory === cat ? 'bg-neon-green text-black border-neon-green' : 'bg-slate-900/40 text-slate-500 border-white/5'}`}
                    >
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {/* 搜尋自訂動作入口：當搜尋不到完全相符的動作時顯示 */}
                {searchTerm.trim() && !isExactMatch && (
                  <motion.button 
                    whileTap={{ scale: 0.95 }} 
                    onClick={() => addExercise(searchTerm.trim())} 
                    className="col-span-2 p-4 rounded-[20px] bg-neon-green/10 border border-neon-green/30 flex items-center justify-between group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-neon-green rounded-xl flex items-center justify-center text-black">
                        <PlusSquare className="w-5 h-5" />
                      </div>
                      <div className="text-left overflow-hidden">
                        <div className="text-[10px] font-black text-neon-green uppercase tracking-widest leading-none">建立自訂動作</div>
                        <div className="text-sm font-black italic text-white uppercase truncate max-w-[200px] mt-1">{searchTerm}</div>
                      </div>
                    </div>
                    <ChevronRight className="w-4 h-4 text-neon-green opacity-50" />
                  </motion.button>
                )}
                ...
                {filteredExercises.map(exName => (
                  <motion.button 
                    ...
                    className="p-3.5 rounded-[20px] text-left bg-slate-900/40 border border-white/5 flex flex-col justify-center min-h-[68px] group active:border-neon-green/30"
                  >
                    <div className="text-[11px] font-black italic uppercase text-slate-200 group-active:text-neon-green leading-tight truncate">
                      {exName}
                    </div>
                    <div className="text-[8px] font-bold text-slate-700 uppercase tracking-widest mt-1.5 flex items-center justify-between">
                      {getMuscleGroupDisplay(getMuscleGroup(exName)).cn}
                      <Plus className="w-3 h-3 text-slate-800 opacity-0 group-active:opacity-100 transition-opacity" />
                    </div>
                  </motion.button>
                ))}
              </div>
          ...
          <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6 pb-40">
            ...
            <div className="w-full relative px-1">
              <div className="relative overflow-hidden rounded-[20px] shadow-2xl border border-white/5 bg-slate-900 min-h-[220px] flex items-center justify-center">
                {currentDetailEx?.name === '啞鈴上斜臥推' ? (
                  ...
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2022/01/developpe-couche-prise-inversee.gif" 
                    alt="槓鈴平板臥推" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '槓鈴上斜臥推' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2021/10/developpe-incline-barre.gif" 
                    alt="槓鈴上斜臥推" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '啞鈴平板臥推' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2000/05/developpe-couche-halteres-exercice-musculation.gif" 
                    alt="啞鈴平板臥推" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '史密斯平板臥推' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2022/08/developpe-couche-smith-machine.gif" 
                    alt="史密斯平板臥推" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '坐姿器械推胸' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2022/11/developpe-machine-assis-pectoraux.gif" 
                    alt="坐姿器械推胸" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '蝴蝶機夾胸' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2000/06/pec-deck-butterfly-exercice-musculation.gif" 
                    alt="蝴蝶機夾胸" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '跪姿繩索夾胸' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2023/07/ecarte-a-la-poulie-vis-a-vis-haute-a-genoux.gif" 
                    alt="跪姿繩索夾胸" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '器械上斜推胸' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2000/06/developpe-incline-machine-convergente-exercice-musculation.gif" 
                    alt="器械上斜推胸" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '史密斯上斜臥推' ? (
                  <img 
                    src="https://fitliferegime.com/wp-content/uploads/2024/04/Smith-Machine-Incline-Press.gif" 
                    alt="史密斯上斜臥推" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '雙槓撐體' ? (
                  <img 
                    src="https://i.pinimg.com/originals/e7/45/d6/e745d6fcd41963a8a6d36c4b66c009a9.gif" 
                    alt="雙槓撐體" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : currentDetailEx?.name === '標準俯地挺身' ? (
                  <img 
                    src="https://www.docteur-fitness.com/wp-content/uploads/2020/10/pompe-musculation.gif" 
                    alt="標準俯地挺身" 
                    style={{ width: '100%', borderRadius: '15px', display: 'block' }} 
                  />
                ) : gifUrl ? (
            ...
            <div className="mx-1 p-5 rounded-[24px] bg-slate-900/40 border border-white/5 space-y-3">
              <div className="flex items-center gap-2 text-neon-green">
                <BookOpen className="w-4 h-4" />
                <h3 className="text-[10px] font-black uppercase tracking-widest">運動方法</h3>
              </div>
              <p className="text-[12px] font-medium text-slate-400 leading-relaxed italic whitespace-pre-line">
            ...
            <div className="space-y-4 px-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-black italic uppercase text-white">訓練錄入</h3>
                </div>
                <button onClick={() => onUpdate({ ...session, exercises: session.exercises.map(ex => ex.id === currentDetailEx!.id ? { ...ex, sets: [...ex.sets, { id: crypto.randomUUID(), weight: ex.sets[ex.sets.length-1]?.weight || 0, reps: ex.sets[ex.sets.length-1]?.reps || 10, completed: false }] } : ex) })} className="flex items-center gap-1 text-neon-green text-[9px] font-black uppercase">
                  <PlusCircle className="w-3.5 h-3.5" /> 加一組
                </button>
              </div>
              ...
              <div className="pt-6">
                <button onClick={onFinish} className="w-full bg-neon-green text-black font-black h-16 rounded-[24px] uppercase italic text-lg active:scale-95 transition-all shadow-xl flex items-center justify-center gap-3">
                  <Save className="w-6 h-6 stroke-[3]" /> 儲存紀錄
                </button>
              </div>
            </div>
          </motion.div>

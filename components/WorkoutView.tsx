        ...
        {!activeExerciseId ? (
          <motion.div key="overview" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6 pb-40">
            ...
            {session.exercises.some(ex => ex.sets.some(s => s.completed)) && (
              <div className="fixed bottom-[110px] left-0 right-0 z-40 px-8 max-w-md mx-auto">
                ...
                >
                  <Save className="w-6 h-6 stroke-[2.5]" /> 結束並儲存本次訓練
                </button>
              </div>
            )}
          </motion.div>
          ...
          <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6 pb-40">
            ...
            <div className="w-full relative px-1">
              {renderExerciseGif(currentDetailEx?.name || "")}
            </div>
                  ...
                  <button onClick={() => {
                    const newExercises = session.exercises.map(ex => {
                      if (ex.id === activeExerciseId) {
                        ...
                        return {
                          ...ex,
                          sets: [...ex.sets, { id: crypto.randomUUID(), weight: lastSet?.weight || 0, reps: lastSet?.reps || 10, completed: false }]
                        };
                      }
                      return ex;
                    });
                    onUpdate({ ...session, exercises: newExercises });
                  }} className="flex items-center gap-1.5 text-neon-green text-[10px] font-black uppercase">
                  ...
                  <div key={set.id} className={`grid grid-cols-12 gap-2.5 items-center p-4 rounded-[28px] border transition-all ${set.completed ? 'bg-neon-green/5 border-neon-green/20' : 'bg-slate-900/60 border-white/5'}`}>
                    <div className="col-span-1 flex justify-center">
                      <button onClick={() => {
                        const newExercises = session.exercises.map(ex => {
                          if (ex.id === activeExerciseId) {
                            return { ...ex, sets: ex.sets.filter(s => s.id !== set.id) };
                          }
                          return ex;
                        });
                        onUpdate({ ...session, exercises: newExercises });
                      }} className="text-slate-800 p-1 active:text-red-500">
                    ...
                    <div className="col-span-4 flex items-center justify-center gap-2">
                      <input 
                        ...
                        onChange={e => {
                          ...
                          const newExercises = session.exercises.map(ex => {
                            if (ex.id === activeExerciseId) {
                              const newSets = ex.sets.map(s => s.id === set.id ? { ...s, reps: nr } : s);
                              return { ...ex, sets: newSets };
                            }
                            return ex;
                          });
                    ...
                    <div className="col-span-2 flex justify-end">
                      <button 
                        onClick={() => {
                          ...
                          if (newStatus && context?.triggerRestTimer) context.triggerRestTimer();
                          const newExercises = session.exercises.map(ex => {
                            if (ex.id === activeExerciseId) {
                              const newSets = ex.sets.map(s => s.id === set.id ? { ...s, completed: newStatus } : s);
                              return { ...ex, sets: newSets };
                            }
                            return ex;
                          });
